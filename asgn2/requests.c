...
void process_request(struct httpObject *message){
    ...
    if(n == -1){warn("no double carraige"); message->valid = false;} 
    else{ 
        cpy += n;
        message->buff[n-1] = '\0';
    ...
    switch(message->method)
    {
        ...
        case GET:
            if(strcmp(message->filename, "healthcheck") == 0){
                //if not log file was created
                if(message->logfd == -1){
                    warn("INVALID HEALTH CHECK. NO LOG WAS CREATED");
                    create_error_response(message, 2); 
                    return;
                }
                printf("PROPER HEALTH CHECK\n");
